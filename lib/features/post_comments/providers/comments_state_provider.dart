...
class PostCommentsState extends StateNotifier<_HelperClass> {
  ...
  PostCommentsState({required Ref ref, required String chapterId})
  ...
  void updateComment(PostCommentModel comment) {
    ...
    List<PostCommentModel> updatedState = List.from(state.comments);
    updatedState[indexOf] = comment;
    updateState(comments: updatedState);
  }

  void addComment(PostCommentModel comment) {
    List<PostCommentModel> updatedState = List.from(state.comments);
  ...
  PostCommentModel getById(String id) {
    return state.comments.firstWhere((c) => c.id == id);
  }
}

final postCommentsStateProvider = StateNotifierProvider.family
    .autoDispose<PostCommentsState, _HelperClass, String>((ref, chapterId) {
      return PostCommentsState(ref: ref, chapterId: chapterId);
    });
