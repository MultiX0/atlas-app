...
class NovelReviewsHelper {
  ...
  NovelReviewsHelper copyWith({
    List<NovelReviewModel>? reviews,
    int? reviewsCount,
    String? error,
    bool? isLoading,
    bool? hasReachedEnd,
...
class NovelReviewsState extends StateNotifier<NovelReviewsHelper> {
  final Ref _ref;
  ...
  NovelReviewsState({required Ref ref, required String novelId})
    : _ref = ref,
      _novelId = novelId,
      super(
        NovelReviewsHelper(
  ...
  Future<void> fetchReviews({bool refresh = false}) async {
    try {
      ...
      if (!state.user_have_review_before) {
        ...
        final reviewd = await _db.checkIhaveComicReview(userId: me.userId, comicId: _novelId);
      ...
      final reviews = await _db.getNovelReviews(
        novelId: _novelId,
  ...
  Future<void> _updateCountAndAvg() async {
    final data = await Future.wait<dynamic>([
      _db.getNovelReviewsCount(_novelId),
      _db.getAvgNovelReviews(_novelId),
    ]);
  ...
  void addReview(NovelReviewModel review) {
    List<NovelReviewModel> newReivews = List.from(state.reviews);
  ...
  void updateReviewByUserId(NovelReviewModel review) {
    final indexOf = state.reviews.indexWhere((rev) => rev.userId == review.userId);
    List<NovelReviewModel> newReivews = List.from(
  ...
  void deleteReview(NovelReviewModel review) {
  ...
  void handleLocalLike(NovelReviewModel review, int index) {
    if (index < 0 || index >= state.reviews.length) return;

    final updated = List<NovelReviewModel>.from(state.reviews);
  ...
  void handleNewRepost(NovelReviewModel review) {
...
final novelReviewsState =
    StateNotifierProvider.family<NovelReviewsState, NovelReviewsHelper, String>((ref, novelId) {
      return NovelReviewsState(ref: ref, novelId: novelId);
    });
