        ...
        appBar: AppBar(
          title: Consumer(
            builder: (context, ref, _) {
              ...
              if (chapter == null) return const Text('null');
            ...
            Expanded(
              child: Consumer(
                builder: (context, ref, _) {
                  final chapter = ref.read(currentChapterProvider);
                  if (chapter == null) {
                    return const AtlasErrorPage(message: "ChapterData is null");
                  }
                  ...
                  final error = ref.watch(notifier.select((s) => s.error));
                  bool hasError = error != null;

                  final loadingMore = ref.watch(notifier.select((s) => s.loadingMore));

                  if (hasError) return AtlasErrorPage(message: error);
